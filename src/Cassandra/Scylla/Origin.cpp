...
bool Origin::getSSLCerts()
{
    ...
    if (cass_future_error_code(future) == CASS_OK)
    {
        ...
        while (cass_iterator_next(iterator))
        {
            ...
            Zone *zone = Gen::Global::zones.findOrCreate(std::string_view(domain, domainLen));
            ...
            Gen::Global::zones.replaceCtx(zone, zoneCtx);
        }
...
bool Origin::loadSSLCertForDomain(const std::string &domain, const std::string &host)
{
    ...
    while (cass_iterator_next(iterator))
    {
        ...
        Zone *zone = Gen::Global::zones.findOrCreate(domain);
        SSL_CTX *zoneCtx = SSL_CTX_new(TLS_server_method());
...
bool Origin::insertStatsSync()
{
    ...
    Gen::Global::zones.forEach([&](const std::string &domain, Zone &zone)
                       {
